...
class HttpGetterTest {
	...
	void init(MockServerClient mockServerClient) throws IOException {
		...
		Mockito.when(config.userAgent()).thenReturn(Optional.of("http-getter-test"));
		Mockito.when(config.httpThreads()).thenReturn(3);
		Mockito.when(config.maxFeedResponseSize()).thenReturn(new MemorySize(new BigInteger("10000")));
